...
const API_BASE_URL = (() => {
    ...
    // For Vercel deployment - point to your deployed backend
    if (window.location.href.includes('vercel.app')) {
        ...
        return 'https://your-backend-url-here.railway.app'; // Update this with your actual backend URL
    }
    // For other production deployments (Netlify, etc.)
    // Demo mode: return localhost for now, will show connection errors gracefully
    return 'http://localhost:8000'; // This will trigger demo mode with connection errors
})();
...
function displayTrendResults(data) {
    ...
    // Ensure results are fully hidden before updating content
    ...
    // Show results with smooth transition after content is ready
    setTimeout(() => {
        elements.trendResults.classList.remove('d-none');
        elements.trendResults.style.display = 'block';
        
        // Store data for export (include form parameters for better export)
        window.lastTrendData = {
            ...data,
            query: elements.trendQuery?.value?.trim(),
            industry: elements.trendIndustry?.value?.trim(),
            timeframe: elements.trendTimeframe?.value?.trim()
        };
...
function displayAudienceResults(data) {
    ...
    // Ensure results are fully hidden before updating content
    elements.audienceResults.style.display = 'none';
    ...
    setTimeout(() => {
        ...
        window.lastAudienceData = {
            ...data,
            target_audience: elements.audienceDescription?.value?.trim(),
            product_category: elements.audienceCategory?.value?.trim(),
            region: elements.audienceRegion?.value?.trim()
        };
...
function exportAsCSV(data, filename) {
    ...
    if (data.query) {
        csvContent += `"Query","${data.query.replace(/"/g, '""')}"\n`;
    }
    if (data.industry) {
        csvContent += `"Industry","${data.industry.replace(/"/g, '""')}"\n`;
    }
    if (data.timeframe) {
        csvContent += `"Timeframe","${data.timeframe.replace(/"/g, '""')}"\n`;
    }
    if (data.target_audience) {
        csvContent += `"Target Audience","${data.target_audience.replace(/"/g, '""')}"\n`;
    }
    if (data.product_category) {
        csvContent += `"Product Category","${data.product_category.replace(/"/g, '""')}"\n`;
    }
    if (data.region) {
        csvContent += `"Region","${data.region.replace(/"/g, '""')}"\n`;
    }
...
function showHelperMenu(button) {
    ...
    // Detect if user is a developer based on environment
    const isDeveloper = (
        // URL parameter for developer mode (works on any domain)
        new URLSearchParams(window.location.search).has('dev') ||
        new URLSearchParams(window.location.search).get('dev') === 'true' ||
    ...
    // Base user tools (available to everyone)
    let menuHTML = `
    ...
    if (isDeveloper) {
        menuHTML += `
            ...
            <div class="helper-item" onclick="testAllFunctionality()">
...
function testAllFunctionality() {
    ...
    // Test 1: Fill trend form with advanced options
    const trendQuery = document.getElementById('trendQuery');
    const trendIndustry = document.getElementById('trendIndustry');
    const trendTimeframe = document.getElementById('trendTimeframe');
    
    if (trendQuery) trendQuery.value = 'Sustainable Fashion';
    if (trendIndustry) trendIndustry.value = 'Fashion';
    if (trendTimeframe) trendTimeframe.value = '1 year';
    
    // Show advanced options if hidden
    const trendAdvanced = document.querySelector('#trendForm .advanced-inputs');
    if (trendAdvanced && trendAdvanced.style.display === 'none') {
        toggleAdvancedInputs('trend');
    }
    
    // Test 2: Fill audience form with advanced options
    const audienceDesc = document.getElementById('audienceDescription');
    const audienceCategory = document.getElementById('audienceCategory');
    const audienceRegion = document.getElementById('audienceRegion');
    
    if (audienceDesc) audienceDesc.value = 'Tech-savvy millennials interested in sustainable living';
    if (audienceCategory) audienceCategory.value = 'Fashion';
    if (audienceRegion) audienceRegion.value = 'North America';
    
    // Show advanced options if hidden
    const audienceAdvanced = document.querySelector('#audienceForm .advanced-inputs');
    if (audienceAdvanced && audienceAdvanced.style.display === 'none') {
        toggleAdvancedInputs('audience');
    }
    
    // Test 3: Load test data for export testing
    setupTestDataAndShow();
    
    showSuccessMessage('✅ All features tested! Forms filled, advanced options shown, test data loaded for export testing.');
    console.log('✅ Test complete - Forms populated with advanced options, test data loaded');
...
function saveInsight(type) {
    ...
    // Create a comprehensive save object with form parameters
    const saveData = {
        type: type,
        timestamp: new Date().toISOString(),
        // Include form parameters based on type
        parameters: type === 'trend' ? {
            query: elements.trendQuery?.value?.trim() || 'N/A',
            industry: elements.trendIndustry?.value?.trim() || null,
            timeframe: elements.trendTimeframe?.value?.trim() || null
        } : {
            target_audience: elements.audienceDescription?.value?.trim() || 'N/A',
            product_category: elements.audienceCategory?.value?.trim() || null,
            region: elements.audienceRegion?.value?.trim() || null
        },
        data: data,
...
function initializeRateLimitDisplay() {
    ...
    // Set initial message
    const initialMessage = "Daily quota: 15 requests available";
